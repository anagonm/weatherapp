...
describe("forecastApi", () => {
  ...
  const forecastApiMockedResponse = {
    "cod": "200",
    "message": 0,
    "cnt": 40,
    "list": [
  ...
  it("returns an error if invalid city", async () => {
    fetch.mockResponseOnce(JSON.stringify(forecastApiCityInvalidMockedResponse));
    const forecastApiResponse = await getForecastByCity(invaliCity);
    expect(forecastApiResponse).toEqual(forecastApiCityInvalidMockedResponse);
  })
